...
def main(ask, psk, fsk1, fsk2):
    ...
    equence_fsk = fsk_modulation(fsk1, fsk2, sequence)
    plot(x, equence_fsk, axis_x="Час, c", axis_y="Амплітуда сигналу", title="Частотна модуляція")
    x_spectrum, spectrum_sequence_fsk = spectrum(equence_fsk)
    ...
    sequence_fsk_noise = equence_fsk + noise
    ...
    error_fsk = noise_stress(sequence, equence_fsk, "FSK", [fsk1, fsk2])
    fig, ax = plt.subplots(figsize=(21 / 2.54, 14 / 2.54))
